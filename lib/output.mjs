// Shared output helpers for the SignatureAPI signatureapi-integrate and
// signatureapi-diagnose skills. This is the source; each skill's copy at
// scripts/lib/output.mjs is generated by `npm run sync-lib` and must not be
// edited directly (see test/lib-sync.test.mjs). Owns the live-key refusal
// gate (requireTestKey) and the read-only mode resolver (resolveKey) — the
// file a `grep -rn requireTestKey` surfaces.
