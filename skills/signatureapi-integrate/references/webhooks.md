...
file as `SIGNATUREAPI_WEBHOOK_SECRET`, not into the chat. Generate an event
with a synthetic test envelope; there is no MCP sample-delivery tool.
`list_webhook_attempts` shows each
delivery, response code and timing (not the response body), which is how you tell "the event fired"
