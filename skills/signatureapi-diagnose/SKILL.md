...
### No webhook arrived
There is **no webhook-delivery log** anywhere. This is a known gap. Split the
question in two. Did the event happen (`GET /envelopes/{envelopeId}/events`)?
Is your endpoint reachable from the internet? Test and live endpoints are
separate. A test envelope never notifies a live endpoint.
